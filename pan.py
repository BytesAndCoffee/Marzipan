    ...
    def addressed(self, params, who, chan):
        ...
        if m:
            if query:
                ...
                return "Sorry {}, but I don't think you have a pantry.. :o".format(
                    who)  # this line keeps showing up. should probably put it in a variable or something
        m = rm.match(params)
        if m:
            if query:
                items = query['items']
                items = [item for item in items if
                         item not in [removed.strip() for removed in m.group('items').split(',')]]
                pantry.update({'user': who}, {'$set': {'items': items}})
...
class Marzipan(IRCClient):
    ...
    def help(self, cmd, *args):
        if not commands.has_key(cmd) or commands[cmd] == None:
            keys = commands.keys()
            return "My current allowed commands are: {} and {}. Use .help <cmd> for more info on each.".format(
                ', '.join(keys[:-1]), keys[-1])
        return commands[cmd]
    ...
    def record(self, params, chan):
        ...
        if m:
            ...
            oid = self.db.recipes.insert(
                SON([('name', m.group('recipe')), ('desc', out), ('whose', m.group('target'))]))
...
if __name__ == "__main__":
    ...
    cli = Marzipan(logchannel="#marzipan", host="irc.foonetic.net", port=6667, nick="pan",
                   real_name="Marzipan, ran by rtmiu (testing)")
    conn = cli.connect()
